// font.rs
//! 8x8 bitmap font for ASCII (U+0000 to U+007F) and Latin-1 (U+00A0 to U+00FF), which has
//! the letters of Swedish and the other Western European languages.
//!
//! From font8x8 by Daniel Hepper (<https://github.com/dhepper/font8x8>), based on the IBM
//! VGA fonts via Marcel Sondaar. Public domain.

/// Glyph width and height in pixels.
pub const GLYPH_SIZE: usize = 8;

/// One row per byte, top to bottom. The least significant bit is the leftmost pixel.
pub type Glyph = [u8; GLYPH_SIZE];

/// The glyph for `c`, or `?` for anything the font doesn't have.
pub fn glyph(c: char) -> &'static Glyph {
    match c as usize {
        index @ 0x00..=0x7F => &FONT[index],
        index @ 0xA0..=0xFF => &LATIN1[index - 0xA0],
        _ => &FONT['?' as usize],
    }
}

/// Whether the pixel at (`x`, `y`) of `glyph` is set.
pub fn is_set(glyph: &Glyph, x: usize, y: usize) -> bool {
    glyph[y] >> x & 1 != 0
}

#[rustfmt::skip]
static FONT: [Glyph; 128] = [
...
/// U+00A0 to U+00FF, from font8x8's `font8x8_ext_latin.h`.
#[rustfmt::skip]
static LATIN1: [Glyph; 96] = [
...
mod tests {
    ...
    #[test]
    fn letters_match_the_documented_encoding() {
        // The example from font8x8's README.
        assert_eq!(*glyph('A'), [0x0C, 0x1E, 0x33, 0x33, 0x3F, 0x33, 0x33, 0x00]);
        // Row 4 of 'A' is "xxxxxx..": the six leftmost pixels.
        assert!((0..6).all(|x| is_set(glyph('A'), x, 4)));
        assert!(!is_set(glyph('A'), 6, 4));
    }

    #[test]
    fn characters_outside_the_font_fall_back_to_question_mark() {
        assert_eq!(glyph('€'), glyph('?'));
        assert_eq!(glyph('\u{9F}'), glyph('?'));
        assert_eq!(*glyph(' '), [0; 8]);
    }

    #[test]
    fn latin1_letters_have_their_own_glyphs() {
        for c in ['å', 'ä', 'ö', 'Å', 'Ä', 'Ö', '§', 'é'] {
            assert_ne!(glyph(c), glyph('?'), "{c}");
        }
        // Indexed right: U+00E5 is font8x8's "a ring".
        assert_eq!(*glyph('å'), [0x0C, 0x0C, 0x1E, 0x30, 0x3E, 0x33, 0x7E, 0x00]);
    }
}
